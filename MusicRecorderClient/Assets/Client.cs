...
public interface ClientHandler {
	...
	void OnNotification(string msgType, string json);
}
...
public class Client {
	...
	private void HandlePacket(OpCode opCode, string payload) {	
		...
		case OpCode.BroadcastMessage:
			...
			break;
		case OpCode.Notify:
			Message notification = JsonUtility.FromJson<Message> (payload);
			mHandler.OnNotification (notification.name, payload);
			break;
		case OpCode.Error:
